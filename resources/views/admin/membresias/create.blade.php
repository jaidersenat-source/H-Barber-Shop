...
<div class="membresias-container">
    <div style="display:flex;align-items:center;gap:1rem;margin-bottom:2rem;flex-wrap:wrap;">
        ...
        <h2 tabindex="0" style="margin:0;">Crear membresía</h2>
    </div>
    ...
    <form action="{{ route('membresias.store') }}" method="POST" enctype="multipart/form-data" novalidate>
        @csrf
            ...
            <div class="form-group">
                ...
                <select id="duracion_meses" name="duracion_meses"
                    ...
                    @foreach([1 => '1 mes', 3 => '3 meses', 6 => '6 meses', 12 => '1 año (12 meses)'] as $val => $label)
                        <option value="{{ $val }}" {{ old('duracion_meses') == $val ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="form-group">
            ...
            <select name="ben_tipo" id="ben_tipo" class="form-control" required aria-required="true"
                ...
                <option value="gratis"     {{ old('ben_tipo', 'gratis')      === 'gratis'    ? 'selected' : '' }}>Gratis (usos limitados)</option>
                <option value="porcentaje" {{ old('ben_tipo')                === 'porcentaje' ? 'selected' : '' }}>Porcentaje de descuento</option>
            </select>
        </div>

        {{-- Usos límite (solo para tipo gratis) --}}
        <div class="form-group" id="campo-usos" style="{{ old('ben_tipo', 'gratis') === 'gratis' ? '' : 'display:none;' }}">
            <label for="ben_usos_limite">Número máximo de usos gratuitos</label>
            <input type="number" id="ben_usos_limite" name="ben_usos_limite"
                   value="{{ old('ben_usos_limite', 1) }}"
                   class="form-control" min="1" placeholder="Ej: 5">
            <small style="color:#666;">0 = sin límite de usos.</small>
        </div>

        {{-- Descuento % (solo para tipo porcentaje) --}}
        <div class="form-group" id="campo-pct" style="{{ old('ben_tipo') === 'porcentaje' ? '' : 'display:none;' }}">
                ...
                @foreach($servicios as $s)
                    <label style="display:flex;align-items:center;gap:0.35rem;background:#f3f4f6;padding:0.4rem 0.75rem;border-radius:6px;cursor:pointer;font-size:0.875rem;">
                        <input type="checkbox" name="servicios_aplicables[]" value="{{ $s->serv_id }}"
                               {{ in_array($s->serv_id, old('servicios_aplicables', [])) ? 'checked' : '' }}>
                        {{ $s->serv_nombre }}
                    </label>
                @endforeach
            </div>
        ...
        <div class="form-group" style="display:flex;align-items:center;gap:0.75rem;">
            <input type="checkbox" id="activo" name="activo" value="1"
                   ...
                   aria-label="Membresía activa (visible en sitio público)">
