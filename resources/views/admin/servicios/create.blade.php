...
@section('content')
@vite(['resources/css/Admin/servicios/create.css'])
<div class="servicios-container" id="modulo-servicios-create">
    <h2 tabindex="0">Crear Servicio o Combo</h2>
    ...
    <form method="POST" action="{{ route('servicios.store') }}" aria-label="Formulario para crear servicio o combo">
        @csrf

        <label for="serv_categoria">Categoría</label>
        <select name="serv_categoria" id="serv_categoria" required tabindex="0" aria-label="Categoría del servicio. Seleccione combos para crear un combo especial.">
            <option value="">Seleccione una categoría</option>
            <option value="cortes" {{ old('serv_categoria')=='cortes' ? 'selected' : '' }}>Cortes de Cabello</option>
            <option value="barba" {{ old('serv_categoria')=='barba' ? 'selected' : '' }}>Servicios de Barba</option>
            <option value="tratamientos" {{ old('serv_categoria')=='tratamientos' ? 'selected' : '' }}>Tratamientos Especiales</option>
            <option value="combos" {{ old('serv_categoria')=='combos' ? 'selected' : '' }}>Combos Especiales</option>
        </select>
        ...
        <fieldset id="combo-servicios-fieldset" class="combo-servicios-fieldset" style="{{ old('serv_categoria') === 'combos' ? '' : 'display:none;' }}" aria-label="Servicios incluidos en el combo">
                ...
                @forelse($serviciosDisponibles as $sd)
                    <label class="combo-servicio-check" for="serv_inc_{{ $sd->serv_id }}">
                        <input
                            ...
                            value="{{ $sd->serv_id }}"
                            {{ in_array($sd->serv_id, old('serv_servicios_incluidos', [])) ? 'checked' : '' }}
                            tabindex="0"
                            aria-label="{{ $sd->serv_nombre }} — {{ $sd->serv_precio }} pesos colombianos — {{ $sd->serv_duracion }} minutos"
                        >
                        <span class="combo-servicio-info">
                            <strong>{{ $sd->serv_nombre }}</strong>
                            <small>{{ ucfirst($sd->serv_categoria) }} &bull; {{ $sd->serv_precio }} pesos colombianos &bull; {{ $sd->serv_duracion }} min</small>
                        </span>
                    </label>
                @empty
        ...
        <input type="text" name="serv_nombre" id="serv_nombre" value="{{ old('serv_nombre') }}" required tabindex="0" aria-label="Nombre del servicio">
        @error('serv_nombre')<span class="error" role="alert">{{ $message }}</span>@enderror

        <label for="serv_descripcion">Descripción</label>
        <textarea name="serv_descripcion" id="serv_descripcion" tabindex="0" aria-label="Descripción del servicio">{{ old('serv_descripcion') }}</textarea>

        <label for="serv_precio">Precio</label>
        <input type="number" step="0.01" name="serv_precio" id="serv_precio" value="{{ old('serv_precio') }}" required tabindex="0" aria-label="Precio del servicio en pesos colombianos">
        @error('serv_precio')<span class="error" role="alert">{{ $message }}</span>@enderror

        <label for="serv_duracion">Duración (minutos)</label>
        <input type="number" name="serv_duracion" id="serv_duracion" value="{{ old('serv_duracion') }}" required tabindex="0" aria-label="Duración en minutos">
        @error('serv_duracion')<span class="error" role="alert">{{ $message }}</span>@enderror

        <label for="serv_descuento">Descuento (%)</label>
        <input type="number" name="serv_descuento" id="serv_descuento" min="0" max="100" step="0.01" value="{{ old('serv_descuento', 0) }}" placeholder="Ej: 10 para 10%" tabindex="0" aria-label="Descuento en porcentaje">

        <div class="botones-form">
            <a href="{{ route('servicios.index') }}" class="btn-volver" tabindex="0" aria-label="Volver a la lista de servicios">&larr; Volver</a>
            <button type="submit" class="btn-guardar" tabindex="0" aria-label="Guardar servicio o combo">Guardar</button>
        </div>
    </form>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function toggleComboSection() {
        ...
        // Desmarcar checkboxes si no es combo
        if (!esCombo) {
