...
<div class="card" role="main" aria-label="Gestión de barberos registrados" id="modulo-personal">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:2rem;">
        <h1 tabindex="0" style="margin:0;font-size:2rem;font-weight:700;color:#1a365d;">Barberos registrados</h1>
        <a href="{{ route('personal.create') }}" class="btn btn-outline-primary crear-btn" tabindex="0" aria-label="Registrar nuevo barbero">
            <span style="font-size:1.3em;font-weight:bold;vertical-align:middle;" aria-hidden="true">&#43;</span> Registrar nuevo barbero
        </a>
    </div>
    ...
    <ul class="sr-only" aria-label="Lista de barberos detallada">
        ...
        <li>
            ...
            @if($p->persona->usuario && $p->persona->usuario->estado !== 'activo')
                <span>El usuario está inactivo. <span role="button" aria-label="Activar usuario {{ $p->persona->per_nombre }} {{ $p->persona->per_apellido }}">Activar usuario</span></span>
            @elseif(!$p->persona->usuario)
                <span>No tiene usuario registrado. <span role="button" aria-label="Registrar usuario para {{ $p->persona->per_nombre }} {{ $p->persona->per_apellido }}">Registrar usuario</span></span>
            @else
                <span>El usuario está activo. <span role="button" aria-label="Desactivar usuario {{ $p->persona->per_nombre }} {{ $p->persona->per_apellido }}">Desactivar usuario</span></span>
            @endif
        </li>
        @endforeach
    </ul>
...
<script>
// Navegación accesible con flechas y Tab para todo el módulo Personal
